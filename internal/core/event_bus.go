...
func (bus *EventBus) Subscribe(userID string, sessionID string, filter *TopicFilter) {
	...
	if oldUserID, ok := bus.userIds[sessionID]; !ok || oldUserID != userID {
		...
		bus.subscriptions[userID][sessionID] = &Subscription{
			session: session, filters: make(map[string]*TopicFilter),
		}
	}
...
func (bus *EventBus) Unsubscribe(userID string, sessionID string, filter *TopicFilter) {
	bus.mux.RLock()
	defer bus.mux.RUnlock()
...
func (s *Subscription) add(filter *TopicFilter) {
	s.mux.Lock()
	defer s.mux.Unlock()
	s.filters[filter.Value] = filter
}

func (s *Subscription) remove(filter *TopicFilter) {
	s.mux.Lock()
	defer s.mux.Unlock()
